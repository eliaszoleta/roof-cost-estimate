...
const TIER_COPY = {
  high: (name) => `Roofing costs in ${name} run above the national average, driven by higher labor rates and cost of living.`,
...
export default function StatePage({ slug }) {
  ...
  const description = `See average roofing costs in ${state.name} for 2026: shingle, metal, tile, flat roofing, repair, inspection, and gutters, adjusted for local labor rates. Get a free instant estimate.`;
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: `Roof Cost in ${state.name}`, item: `https://roofingcal.com/roof-cost/${state.slug}` },
    ],
  };
  const faqSchema = {
    '@context': 'https://schema.org',
  ...
  return (
    <>
      <Helmet>
        ...
        <link rel="canonical" href={`https://roofingcal.com/roof-cost/${state.slug}`} />
      ...
      <div style={{ background: '#f8fafc', minHeight: '100vh', padding: '40px 24px 64px' }}>
        <div style={{ maxWidth: 780, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', gap: 6, fontSize: 13, color: '#94a3b8', marginBottom: 24, flexWrap: 'wrap' }}>
            ...
            <span>›</span>
            <span style={{ color: '#0f172a' }}>Roof Cost in {state.name}</span>
          </div>
          ...
          <div style={{ background: 'white', borderRadius: 14, border: '1px solid #e2e8f0', padding: '32px 36px', marginBottom: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, color: PRIMARY, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{state.name}</span>
            </div>
            <h1 style={{ fontSize: 'clamp(24px,4vw,32px)', fontWeight: 800, color: '#0f172a', lineHeight: 1.25, marginBottom: 10 }}>Roofing Cost in {state.name} (2026)</h1>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 30, fontWeight: 800, color: PRIMARY }}>{formatPrice(state.low)} – {formatPrice(state.high)}</span>
          ...
          <div style={{ background: 'white', borderRadius: 14, border: '1px solid #e2e8f0', padding: '32px 36px', marginBottom: 24 }}>
            ...
            <p style={{ fontSize: 13.5, color: '#64748b', marginBottom: 4 }}>Estimated typical job cost, adjusted for {state.name}'s labor rates. <a href={url('/how-we-calculate-prices')} style={{ color: PRIMARY, fontWeight: 600 }}>See how we calculate these prices →</a></p>
          ...
          <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 12, padding: '24px 28px', marginBottom: 32, textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 14, color: '#64748b', marginBottom: 16 }}>Use our free calculator for a ZIP-code accurate roofing estimate in {state.name} in under 60 seconds.</p>
          ...
          {cities.length > 0 && (
            <div style={{ marginBottom: 32 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 16 }}>Roofing Costs by City in {state.name}</div>
          ...
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 16 }}>Roofing Costs in Other States</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {otherStates.map(s => (
                <a key={s.code} href={url(`/roof-cost/${s.slug}`)} style={{ fontSize: 12.5, color: '#64748b', textDecoration: 'none', background: 'white', border: '1px solid #e2e8f0', borderRadius: 20, padding: '6px 12px' }}>
                  {s.name}
                </a>
              ))}
            </div>
